...
bool ImGui::ImageTabButton(ImTextureID user_texture_id, const ImVec2& size, const bool active, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderFrame(bb.Min, bb.Max, GetColorU32(ImVec4(0.16f, 0.14f, 0.28f, 1.f)), true, style.FrameRounding);
    if (active)
        RenderFrame(bb.Min, bb.Max, GetColorU32(ImVec4(0.21f, 0.19f, 0.38f, 1.f)), true, style.FrameRounding);
